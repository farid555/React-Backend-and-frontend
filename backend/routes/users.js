...
router.delete(`/:id`, async (req, res) => {
    User.findByIdAndRemove(req.params.id).then(user => {
        if (user) {
            return res.status(200).json({ success: true, message: 'the user is deleted!' })// success deleted 
        } else {
            return res.status(404).json({ success: false, message: 'user not found!' })// find no found code
        }
